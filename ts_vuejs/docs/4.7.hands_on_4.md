# PasswordGenerator.vueを追加しよう
メイン処理を記述してある、PasswordGenerator.vue ファイルを追加します。

## SymbolSelector.vue、StackList.vue、ファイルをインポーします
ボタン処理用のコンポーネントとスタック処理用のコンポーネントを読み込む様にします。

```javascript:PasswordGenerator.vue
<template>
    <div>
        <label>パスワード桁数： </label>
